...
export class OfertaPage implements OnInit {
  ...
  constructor(private router: Router, public http: Http, public alerCtrl: AlertController, private theme: ThemeService) {
    this.getMaterias();
  }
  ...
  getMaterias() {
    return new Promise(resolve => {
      ...
      .pipe(map(res => res.json())).subscribe(items => {
        ...
        this.itemst1 = items.filter((item) => item.T1 === true && item.name !== 'Electiva 1'
        && item.name !== 'Electiva 2' && item.name !== 'Electiva 3'
        && item.name !== 'Electiva 4' && item.name !== 'Electiva 5'
        && item.name !== 'Electiva 6' && item.name !== 'Electiva 7');

        this.itemst2 = items.filter((item) => item.T2 === true && item.name !== 'Electiva 1'
        && item.name !== 'Electiva 2' && item.name !== 'Electiva 3'
        && item.name !== 'Electiva 4' && item.name !== 'Electiva 5'
        && item.name !== 'Electiva 6' && item.name !== 'Electiva 7');

        this.itemst3 = items.filter((item) => item.T3 === true && item.name !== 'Electiva 1'
        && item.name !== 'Electiva 2' && item.name !== 'Electiva 3'
        && item.name !== 'Electiva 4' && item.name !== 'Electiva 5'
        && item.name !== 'Electiva 6' && item.name !== 'Electiva 7');
        resolve(this.items);
...
getItems(ev) {
    ...
    este metodo filtrara todas las materias tanto por trimestre en que abren como mostrar en
    itemsb las mas relevantes de acuerdo a la busqueda que luego seran mostradas al comienzo */
      if (val && val.trim() !== '') {
